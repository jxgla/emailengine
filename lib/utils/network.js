...
const GOOGLE_CRAWLER_RANGES_PATH = pathlib.join(__dirname, '..', '..', 'data', 'google-crawlers.json');

function loadGoogleCrawlerRanges() {
    try {
        let ranges = require(GOOGLE_CRAWLER_RANGES_PATH);
        if (!ranges || !Array.isArray(ranges.prefixes)) {
            logger.warn({
                msg: 'Invalid Google crawler ranges file, using empty prefix list',
                path: GOOGLE_CRAWLER_RANGES_PATH
            });
            return { prefixes: [] };
        }
        return ranges;
        ...
        logger.warn({
            msg: 'Failed to load Google crawler ranges file, automated scanner detection will use DNS checks only',
            path: GOOGLE_CRAWLER_RANGES_PATH,
            err
        });
        return { prefixes: [] };
...
const googleCrawlerRanges = loadGoogleCrawlerRanges();
    ...
    for (let prefixKey of ['ipv6Prefix', 'ipv4Prefix']) {
        if (prefixEntry[prefixKey]) {
            try {
                ...
                logger.warn({
                    msg: 'Failed to parse Google crawler IP range',
                    prefix: prefixEntry[prefixKey],
                    err
                });
            }
        }
