...
const { buildRemoteManagedOutlookAccount, importRemoteOutlookAccountsToAuthServer, parseRemoteOutlookAccountLines } = require('./remote-outlook');
...
-----END LICENSE-----`;
    ...
    const canReadMailbox = accountData => (accountData.imap || accountData.oauth2) && !(accountData.imap && accountData.imap.disabled) && !DISABLE_MESSAGE_BROWSER;

    const getOutlookAuthServerApps = async selectedApp => {
        const apps = (await oauth2Apps.list(0, 1000)).apps.filter(
            app => app.enabled && app.provider === 'outlook' && ['api', 'imap'].includes(app.baseScopes)
        );

        return apps.map(app => {
            const cloudInfo = AZURE_CLOUDS.find(entry => entry.id === (app.cloud || 'global'));
            return Object.assign({}, app, {
                selected: app.id === selectedApp,
                cloudLabel: (cloudInfo && cloudInfo.name) || app.cloud || 'global',
                scopeLabel: app.baseScopes === 'imap' ? 'IMAP auth server' : 'Graph auth server'
            });
        });
    };

    const getAuthServerDisplayValue = async () => {
        const authServer = await settings.get('authServer');
        if (!authServer) {
            return {
                configured: false,
                value: ''
    ...
    const renderOutlookAuthServerImportPage = async (request, h, viewModel = {}) => {
        const authServerInfo = await getAuthServerDisplayValue();
        const defaults = {
            app: '',
            accountIdPrefix: '',
            proxy: '',
        ...
        const values = Object.assign({}, defaults, viewModel.values || {});
        let outlookApps = await getOutlookAuthServerApps(values.app);
        if (!values.app && outlookApps[0]) {
            values.app = outlookApps[0].id;
            outlookApps = await getOutlookAuthServerApps(values.app);
        }

        return h.view(
            'accounts/import-outlook-auth-server',
            {
                pageTitle: 'Batch Import Outlook',
                menuAccounts: true,
                authServerConfigured: authServerInfo.configured,
                authServerValue: authServerInfo.value,
                outlookApps,
                values,
                errors: viewModel.errors || {},
                result: viewModel.result || null
            },
            {
                layout: 'app'
            }
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            const formattedAccounts = accounts.accounts.map(account => {
                const formattedAccount = formatAccountData(account.data || account, request.app.gt);
                formattedAccount.canReadMail = canReadMailbox(formattedAccount);
                formattedAccount.mailboxUrl = formattedAccount.canReadMail
                    ? `/admin/accounts/${encodeURIComponent(formattedAccount.account)}/browse`
                    : false;
    ...
    server.route({
        method: 'GET',
        path: '/admin/accounts/import/outlook-auth-server',

        async handler(request, h) {
            return renderOutlookAuthServerImportPage(request, h);
        }
    });
        ...
        async handler(request, h) {
            ...
            if (!authServer) {
                return renderOutlookAuthServerImportPage(request, h, {
                    values,
                    errors: {
                        submit: 'Authentication server is not configured.'
                    }
                });
            }

            const oauth2App = await oauth2Apps.get(request.payload.app);
            if (!oauth2App || !oauth2App.enabled) {
                return renderOutlookAuthServerImportPage(request, h, {
                    values,
                    errors: {
                        app: 'Select an enabled Outlook OAuth application.'
                    }
                });
            }

            if (oauth2App.provider !== 'outlook' || !['api', 'imap'].includes(oauth2App.baseScopes)) {
                return renderOutlookAuthServerImportPage(request, h, {
                    values,
                    errors: {
                        app: 'Selected OAuth app must use the Outlook provider with baseScopes=api or baseScopes=imap.'
                    }
                });
            }
            ...
            if (!request.payload.dryRun && entries.length) {
                try {
                    broker = await importRemoteOutlookAccountsToAuthServer(authServer, entries, {
                        accountIdPrefix: request.payload.accountIdPrefix,
                        tenant: oauth2App.authority || 'common',
                        proxyUrl: request.payload.proxy,
                        cloud: oauth2App.cloud || 'global',
                        baseScopes: oauth2App.baseScopes || 'api'
                    });
                } catch (err) {
                    request.logger.error({ msg: 'Failed to sync imported Outlook accounts to auth server', err });
                    return renderOutlookAuthServerImportPage(request, h, {
                        values,
                        errors: {
                            submit: err.message
                        ...
                        result: {
                            ...
                            summary: {
                                ...
                                created: 0,
                                updated: 0,
                                failed: summary.parsed + errors.length
                            },
                            results: [],
                            errors
                        }
                ...
                for (let entry of entries) {
                    ...
                    try {
                        const accountData = buildRemoteManagedOutlookAccount(entry, {
                            app: request.payload.app,
                            baseScopes: oauth2App.baseScopes || 'api',
                            cloud: oauth2App.cloud || 'global',
                            defaults: {
                        ...
                        const result = await accountManager.create(accountData);
                        if (result.state === 'new') {
                            created++;
                        } else {
                            updated++;
                        ...
                        results.push({
                            ...
                            email: entry.email,
                            state: result.state,
                            accountUrl: `/admin/accounts/${encodeURIComponent(result.account)}`,
                            mailboxUrl: `/admin/accounts/${encodeURIComponent(result.account)}/browse`
                        });
                    } catch (err) {
                        request.logger.error({ msg: 'Batch Outlook auth server import failed', account: entry.account, email: entry.email, err });
                        errors.push({
                            line: entry.line,
                            account: entry.account,
                            email: entry.email,
                            error: err.message,
                            code: (err.output && err.output.payload && err.output.payload.code) || err.code || null
                        });
                    }
                }
            } else {
        ...
        options: {
            validate: {
                ...
                async failAction(request, h, err) {
                    ...
                    const response = await renderOutlookAuthServerImportPage(request, h, {
                        values: {
                            app: request.payload && request.payload.app,
                            accountIdPrefix: request.payload && request.payload.accountIdPrefix,
                            proxy: request.payload && request.payload.proxy,
                            logs: !!(request.payload && request.payload.logs),
                            dryRun: !!(request.payload && request.payload.dryRun),
                            accounts: (request.payload && request.payload.accounts) || ''
                        },
                        errors
                    });
                ...
                payload: Joi.object({
                    app: Joi.string().max(256).required(),
                    accounts: Joi.string().max(1024 * 1024).required(),
                    accountIdPrefix: Joi.string().empty('').trim().max(128).default(''),
                    proxy: settingsSchema.proxyUrl.allow('').default(''),
                    logs: Joi.boolean().truthy('Y', 'true', '1', 'on').falsy('N', 'false', 0, '').default(false),
                    dryRun: Joi.boolean().truthy('Y', 'true', '1', 'on').falsy('N', 'false', 0, '').default(false)
                })
            }
        }
    });
