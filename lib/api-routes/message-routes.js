...
const verificationCandidateSchema = Joi.object({
    slot: Joi.string().valid('inbox', 'junk').required().example('inbox').description('Which folder candidate this message came from'),
    path: Joi.string().required().example('\\Inbox').description('Mailbox path used for lookup'),
    name: Joi.string().allow('').required().example('Inbox').description('Display name for the mailbox'),
    specialUse: Joi.array()
        .items(Joi.string().example('\\Inbox'))
        .required()
        .description('Mailbox special use flags'),
    messageId: Joi.string().allow('').required().example('AAAAAQAACnA').description('Message ID of the candidate email'),
    messageDate: Joi.string().allow('').required().example('2026-04-10T10:00:00.000Z').description('Message timestamp string'),
    messageTimestamp: Joi.number().integer().required().example(1775815200000).description('Message timestamp in milliseconds'),
    messageSubject: Joi.string().allow('').required().example('Your verification code').description('Subject of the candidate email'),
    messageFrom: Joi.string().allow('').required().example('sender@example.com').description('Sender address of the candidate email')
}).label('VerificationCandidate');

async function getLatestVerificationCandidate(accountObject, slot, path, displayName) {
    let listResponse;
    try {
        listResponse = await accountObject.listMessages({
            path,
            pageSize: 1
        });
    } catch (err) {
        if (err && (err.statusCode === 404 || err.code === 'NotFound' || err.code === 'MAILBOX_NOT_FOUND')) {
            return null;
        }
        throw err;
    }

    const message = listResponse && Array.isArray(listResponse.messages) ? listResponse.messages[0] : null;
    return summarizeCandidate(slot, { path, name: displayName, specialUse: path }, message);
}

async function getVerificationMessageText(accountObject, candidate) {
    if (!candidate || !candidate.messageId) {
        return null;
    }

    const message = await accountObject.getMessage(candidate.messageId, {
        textType: '*'
    });

    if (message && message.text && message.text.id) {
        const textContent = await accountObject.getText(message.text.id, {
            textType: '*'
        });

        return Object.assign({}, message, {
            text: Object.assign({}, message.text, textContent || {})
        });
    }
...
async function init(args) {
    ...
    // GET /v1/account/{account}/verification-code - Extract verification code from newest Inbox/Junk message
    server.route({
        method: 'GET',
        path: '/v1/account/{account}/verification-code',
        ...
        async handler(request, h) {
            ...
            try {
                const candidates = {
                    inbox: await getLatestVerificationCandidate(accountObject, 'inbox', '\\Inbox', 'Inbox'),
                    junk: await getLatestVerificationCandidate(accountObject, 'junk', '\\Junk', 'Junk')
                };

                const selectedCandidate = pickNewestMessageCandidate([candidates.inbox, candidates.junk]);
                if (!selectedCandidate) {
                    throw createVerificationError(
                        'VERIFICATION_MESSAGE_NOT_FOUND',
                        'No messages were found in Inbox or Junk for verification code extraction',
                        404
                    );
                }

                const verificationMessage = await getVerificationMessageText(accountObject, selectedCandidate);
                const extracted = extractVerificationCodeFromMessage(verificationMessage, {
                    codeRegex: request.query.codeRegex,
                    codeFlags: request.query.codeFlags
                });

                if (!extracted.code) {
                    throw createVerificationError(
                        'VERIFICATION_CODE_NOT_FOUND',
                        'Could not extract a verification code from the newest Inbox or Junk message',
                        404
                    );
                }
                ...
                return {
                    ...
                    matchSource: extracted.matchSource,
                    path: selectedCandidate.path,
                    folderName: selectedCandidate.name,
                    messageId: selectedCandidate.messageId,
                    messageDate: selectedCandidate.messageDate,
                    messageTimestamp: selectedCandidate.messageTimestamp,
                    messageSubject: selectedCandidate.messageSubject,
                    messageFrom: selectedCandidate.messageFrom,
                    selectedSlot: selectedCandidate.slot,
                    candidates
                };
            } catch (err) {
                request.logger.error({ msg: 'API request failed', err });
        ...
        options: {
            description: 'Extract verification code',
            notes:
                'Fetches the newest message from Inbox and Junk folders, compares the timestamps, then extracts the first matching verification code from the newer email.',
            tags: ['api', 'Message'],
            ...
            validate: {
                ...
                query: Joi.object({
                    codeRegex: Joi.string()
                        .trim()
                        .empty('')
                        .max(512)
                        .default(DEFAULT_VERIFICATION_CODE_REGEX)
                        .example('\\b\\d{6}\\b')
                        .description('Custom regular expression for extracting the verification code'),
                    codeFlags: Joi.string()
                        .trim()
                        .empty('')
                        .max(16)
                        .pattern(/^[dgimsuvy]*$/)
                        ...
                        .example('i')
                        .description('JavaScript regular expression flags for codeRegex')
                }).label('VerificationCodeQuery')
            },

            response: {
                schema: Joi.object({
                    account: accountIdSchema.required(),
                    code: Joi.string().required().example('123456').description('Extracted verification code'),
                    regex: Joi.string().required().example('\\b\\d{4,8}\\b').description('Effective regex used for extraction'),
                    flags: Joi.string().required().example('i').description('Effective regex flags used for extraction'),
                    matchSource: Joi.string()
                        .valid('plain', 'html', 'subject', 'combined')
                        .required()
                        .example('plain')
                        .description('Which part of the selected message matched the regex'),
                    path: Joi.string().required().example('\\Inbox').description('Mailbox path of the selected message'),
                    folderName: Joi.string().allow('').required().example('Inbox').description('Display name of the selected mailbox'),
                    messageId: Joi.string().required().example('AAAAAQAACnA').description('ID of the selected message'),
                    messageDate: Joi.string().allow('').required().example('2026-04-10T10:00:00.000Z').description('Date string of the selected message'),
                    messageTimestamp: Joi.number().integer().required().example(1775815200000).description('Timestamp of the selected message in milliseconds'),
                    messageSubject: Joi.string().allow('').required().example('Your verification code').description('Subject of the selected message'),
                    messageFrom: Joi.string().allow('').required().example('sender@example.com').description('Sender of the selected message'),
                    selectedSlot: Joi.string()
                        .valid('inbox', 'junk')
                        .required()
                        .example('junk')
                        .description('Which folder won the newest-message comparison'),
                    candidates: Joi.object({
                        inbox: verificationCandidateSchema.allow(null),
                        junk: verificationCandidateSchema.allow(null)
                    })
                        .required()
                        .label('VerificationCandidates')
                }).label('VerificationCodeResponse'),
                failAction: 'log'
            }
        }
    });
