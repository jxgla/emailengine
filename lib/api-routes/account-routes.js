...
async function init(args) {
    ...
    server.route({
        method: 'POST',
        path: '/v1/accounts/import/outlook-auth-server',

        async handler(request) {
            const authServer = await settings.get('authServer');
            if (!authServer) {
                throw Boom.badRequest('Authentication server is not configured');
            }

            const oauth2App = await oauth2Apps.get(request.payload.app);
            if (!oauth2App) {
                throw Boom.notFound('Missing or disabled OAuth2 app');
            }

            if (oauth2App.provider !== 'outlook') {
                throw Boom.badRequest('Selected OAuth2 app must use the Outlook provider');
            }

            if (!['api', 'imap'].includes(oauth2App.baseScopes)) {
                throw Boom.badRequest('Selected OAuth2 app must have baseScopes=api or baseScopes=imap');
            }
            ...
            if (!request.payload.dryRun) {
                ...
                for (let entry of entries) {
                    ...
                    try {
                        let accountData = buildRemoteManagedOutlookAccount(entry, {
                        ...
                        validateDelegationFields(accountData);
                        ...
                        errors.push({
                            ...
                            email: entry.email,
                            error: err.message,
                            code: err.output?.payload?.code || err.code || null
                        });
                    }
                }
            } else {
        ...
        options: {
            description: 'Batch import Outlook accounts managed by an external auth server',
            notes: 'Imports multiple Outlook Graph accounts without storing refresh tokens in EmailEngine',
            tags: ['api', 'Account'],
            ...
            validate: {
                ...
                payload: Joi.object({
                    app: Joi.string().max(256).required().description('OAuth2 Application ID configured in EmailEngine'),
                    accounts: Joi.string()
                        .max(1024 * 1024)
                        .required()
                        .description('Multi-line input using email----password----client_id----refresh_token format'),
                    accountIdPrefix: Joi.string().empty('').trim().max(128).default('').description('Optional prefix added to every generated account ID'),
                    dryRun: Joi.boolean().default(false).description('Validate input without creating or updating accounts'),
                    logs: Joi.boolean().default(false).description('Store recent logs for imported accounts'),
                    notifyFrom: accountSchemas.notifyFrom.default('now'),
                    syncFrom: accountSchemas.syncFrom.default(null),
                    path: accountPathSchema.example(['*']).label('AccountPath'),
                    proxy: settingsSchema.proxyUrl,
                    ...
                    locale: Joi.string().empty('').max(100).example('fr').description('Optional locale'),
                    tz: Joi.string().empty('').max(100).example('Europe/Tallinn').description('Optional timezone')
                }).label('BatchImportOutlookAuthServer')
            }
        }
    });
