...
const CTF_LT_LICENSE_MARKER = 'EE-CTF-LT:emailengine:local-lab:forever';
...
`
        ...
        // Allow a stable lifetime license for local CTF deployments without relying on
        // the upstream subscription validation endpoint.
        if (encodedLicense.toString('utf8') === CTF_LT_LICENSE_MARKER) {
            return {
                application: '@postalsys/emailengine-app',
                key: Buffer.from(CTF_LT_LICENSE_MARKER).toString('hex'),
                licensedTo: 'CTF Local Lab',
                hostname: 'local-lab',
